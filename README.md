...
**想自己验一遍**：`case/` 下有五个版本，从 44 行到 169 行，对应书里的几个关键节点。跑起来：
...
一本速记应用。它从「打开就能写」的 44 行开始，被 AI 重构到 149 行七个文件，最后被修剪到 169 行六个文件，**而且终点能撑的功能比起点多得多**（六种渲染语法、标签解析、可切换的开关）。

读者能看着抽象从无到有、从有到滥、最后被放到该放的地方。这是按模式分类的书做不到的，它们的例子永远停在「设计得刚刚好」的那一刻。

## 状态

**初稿完成，经过三轮独立审读修订。** 17 章 + 2 个附录，约 4.4 万字。

三轮审读分别找出了 11、8、5 个问题，都已修完——包括一个事实错误的语言史论断、一处整段重复、三处编造的证据（虚构的消费者、虚构的测试）。过程和教训记在 [notes/how-this-book-was-made.md](notes/how-this-book-was-made.md)。
