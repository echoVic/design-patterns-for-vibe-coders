...
原件在 `case/ai-answers/editor-refactor/`。你可以对着那 1199 行问一遍：这里面哪几行，是今天就用得上的？
...
`render()` 重画整个列表，看起来「很浪费」。但**这是可以量的，不用猜**：
...
**在感到痛之前不要预设解法。** 这是第 00 章那个 149 行版本的教训换一个场景重演：为想象中的规模付成本，和为一个想象中的需求加抽象，是同一件事。
