...
案例的完整演进在 `case/`。`v0.1` 是那 44 行，`v0.2` 是那 149 行，`v1.1` 是终点——五个版本里挑了四个关键节点留了代码，不是每章一版（为什么见 `case/README.md`）。你可以逐行比一遍，看看还有哪些我没提到的东西。
...
GoF 那本书有 23 个模式，上面只覆盖了十个。不是遗漏，是有意的。
