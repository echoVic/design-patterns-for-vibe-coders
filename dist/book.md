...
速记应用长到一定程度，编辑器需要配置了：主题、字号、行距、内容宽度、自动保存延迟、最大笔记数，还有四种语法开关。
...
而且它们有一个共同的性质：每个都可以单独关掉。 不想要日志就去掉 `withLog`，其他三层不受影响。这是包装最值钱的地方——它把四件正交的事情拆成了四块。
...
速记应用加了一个新的操作：点笔记右上角的时间，可以改这条笔记的归属日期。
...
速记应用的三个地方都要读写存储：主界面、设置面板、导出。三处的错误处理长这样：
