...
- 换存储方式（换一个同样是同步的键值存储）：v0.2 只改一个文件，v0.1 要改散落各处的 `localStorage` 调用
- 加一个笔记字段：v0.2 要经手三个文件，v0.1 只改一处
...
（表里「换存储」那一格说的都是**约束不变**的情况。如果新实现和旧实现的约束不同（比如同步换异步），v0.2 那个接口就废了，要改四处。这是第 04 章和最后一张表的事。）
...
但它给的不止这些。产出是四个文件、187 行：

```
rules.ts     54 行   六条规则一张表 + 由表推导出的开关类型
render.ts    26 行   转义 + 渲染管线
index.ts     12 行   公开入口
parity.ts    95 行   和原实现逐字节对拍的脚本（2594 组输入）
```

核心那两部分加起来 80 行。剩下 107 行是入口文件和一个相当认真的验证脚本。
...
这一版的核心是 80 行的那个量级（一份规则表加一个渲染函数），输出和它逐字节一致。案例里有对照脚本，你可以自己验一遍：
...
![左栏是同类改动：加第七种语法、加第三个存储实现、多加一处 new，它们应该只改一两个文件，改到四个以上就说明那个维度缺一层抽象。右栏是新维度：加搜索、加标签、加图片，它们穿过所有层是正常的。](../figures/fig-11-two-kinds-of-change.svg)
...
这一章的情况 `v1.1` 里没有：它的渲染在一个函数里，没有拆开，也就没有参数往下传的问题。

所以我把它放大试了一次：照着「这个应用再长一截」的样子，加了设置面板、导出、统计三个模块，渲染拆成三层。写完量了一遍，然后把代码从版本里撤掉了（留在 `case/measurements/`，不进 `v0.1` 到 `v1.1` 那条线）。

下面是量出来的东西。三个类型先说明一下：

- `AppState`：当前界面的状态（有哪些笔记、选中哪条）
- `Env`：环境，「整个程序里同一个」的那些东西（日志器、渲染配置、存储客户端）
...
function renderNoteList(notes: Note[], selectedId: string, env: Env, handlers: Handlers) {
  return notes.map((n) => render(n.text, env.renderOptions)).join('')
}
```

**中间那层 `renderSidebar` 的函数体里，`env` 的字段一次都没被读过。** 它只做一件事：从 `state` 里取出两个字段，再把 `env` 和 `handlers` 原样递给下一层。

整条链量下来：`env` 的字段被真正用到 3 次，被「只是往下搬」6 次。**搬运是用它的两倍。**
...
这一改省掉了多少？我照这个写法把刚才那版重写了一遍，同样的功能，只换环境的进法：

| | 一路传 | 注入一次 |
|---|---|---|
| 中间层 `renderSidebar` 的签名 | `(state, env, handlers)` | `(state, handlers)` |
| 它的函数体里读 `env` 字段 | 0 次 | —— |
| 渲染链路上带 `env` 参数的函数 | 4 个 | 2 个，且都只在创建时传 |
| `env` 字段被用到 | 3 次 | 3 次 |
| `env` 只是被往下搬 | 6 次 | 3 次 |

用到的次数没变，搬运从 6 次降到 3 次。中间那一层不再需要为一个它不用的东西负责。
...
结构性模式（适配器、桥接、组合、外观、享元、代理）都没有单独展开。原因不是它们不重要，是**在 TypeScript 里这些模式经常塌缩成语言特性**：
