...
同样的功能，从 44 行 1 个文件，变成 150 行 7 个文件。这是「帮我重构得专业点」的典型产出。
...
src/domain/note-service.ts              30 行
src/main.ts                             42 行
                                       ─────
                                       150 行
```
...
文件从 1 个变成 7 个、行数从 44 涨到 150，安全细节悄无声息地没了。这不是故意埋的——写 v0.2 时注意力全在分层上，那个函数就漏掉了。
